...
export function DashboardPage() {
  const { data, isLoading, error, refetch } = useQuery(() => dashboardService.getDashboardStats(), []);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Live statistics computed from the database - nothing here is hard-coded.</p>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div className="card card-padded">
            ...
            <BarList
              items={[
                ...
                { label: 'In progress', value: statusCounts.in_progress, color: 'var(--color-warning)' },
          ...
          <div className="card card-padded">
            ...
            {agentWorkload.length === 0 ? (
              <EmptyState title="No agents yet" message="Add agents from the Users page to see workload here." />
            ) : (
          ...
          <div className="card card-padded">
            ...
            <dl>
              <div className="detail-row">
                ...
                <dd>{sla.slaComplianceRate !== null ? `${sla.slaComplianceRate}%` : 'No resolved tickets yet'}</dd>
              </div>
              <div className="detail-row">
              ...
              <div className="detail-row">
                ...
                <dd>{performance.averageResolutionHours !== null ? `${performance.averageResolutionHours}h` : '—'}</dd>
              </div>
              <div className="detail-row">
            ...
            {recentActivity.length === 0 ? (
              ...
              <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                {recentActivity.map((ticket) => (
                  <li key={ticket._id} style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10 }}>
                    <Link to={`/tickets/${ticket._id}`} style={{ fontWeight: 600, fontSize: '0.875rem' }}>
...
function StatCard({ label, value, accent }: { label: string; value: number; accent?: 'danger' | 'warning' }) {
  return (
    <div className="stat-card">
      <div className="stat-card-label">{label}</div>
      <div
        className="stat-card-value"
        style={{ color: accent === 'danger' ? 'var(--color-danger)' : accent === 'warning' ? 'var(--color-warning)' : undefined }}
      >
        {value}
      </div>
    </div>
